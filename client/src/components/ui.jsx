...
export function PageHeader({ eyebrow, title, subtitle, action }) {
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        {eyebrow && (
          <p className="mb-1 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-leaf)]">
            {eyebrow}
          </p>
        )}
...
export function Panel({ children, className = '' }) {
  return <div className={`glass-panel rounded-2xl p-5 sm:p-6 ${className}`}>{children}</div>;
}
...
export function Alert({ tone = 'danger', children }) {
  const styles =
    tone === 'ok'
      ? 'border-[color-mix(in_srgb,var(--color-ok)_40%,var(--color-line))] bg-[color-mix(in_srgb,var(--color-ok)_12%,var(--color-booth))] text-[var(--color-ok)]'
      : 'border-[color-mix(in_srgb,var(--color-danger)_40%,var(--color-line))] bg-[color-mix(in_srgb,var(--color-danger)_12%,var(--color-booth))] text-[var(--color-danger)]';
  return (
...
export function TopicCloud({ title, topics, empty, tone = 'weak' }) {
  const chip =
    tone === 'strong'
      ? 'bg-[color-mix(in_srgb,var(--color-amber)_16%,var(--color-booth))] text-[var(--color-lamp)] border border-[color-mix(in_srgb,var(--color-amber)_30%,var(--color-line))]'
      : 'bg-[rgba(255,255,255,0.05)] text-[var(--color-ink)] border border-[var(--color-line)]';
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {topics?.length ? (
          topics.map((t) => (
            <span key={t} className={`rounded-full px-3 py-1 text-xs font-medium ${chip}`}>
              {t}
            </span>
          ))
        ) : (
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2">
      {items.map(([label, value]) => (
        <div
          key={label}
          className="flex items-center justify-between rounded-xl bg-[var(--color-fog)] px-3 py-2 text-sm"
        >
